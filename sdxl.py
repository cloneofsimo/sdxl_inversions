...
class Attention(nn.Module):
    def __init__(
        ...
        self.alpha = 1.0
        self.consume_attn_buffer = False
        self.produce_attn_buffer = False
    ...
    def forward(self, hidden_states, encoder_hidden_states=None):
        ...
        if self.consume_attn_buffer:
            attn_weights = self.attn_buffer * self.alpha + attn_weights * (
                1 - self.alpha
            )
        if self.produce_attn_buffer:
            self.attn_buffer = attn_weights
...
class UNet2DConditionModel(ModelMixin, nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(
        ...
        if kwargs.get("decoder_hidden_states", False):
            c9, c10, c11, c12 = kwargs["decoder_hidden_states"]
            alpha = kwargs["alpha"]
        # 4. mid
        s9 = self.mid_block(sample, emb, encoder_hidden_states=encoder_hidden_states)

        if kwargs.get("decoder_hidden_states", False):
            s9 = s9 * alpha + c9 * (1 - alpha)

        # 5. up
        s10 = self.up_blocks[0](
            hidden_states=s9,
            temb=emb,
        ...
        if kwargs.get("decoder_hidden_states", False):
            s10 = s10 * alpha + c10 * (1 - alpha)

        s11 = self.up_blocks[1](
            hidden_states=s10,
            temb=emb,
        ...
        if kwargs.get("decoder_hidden_states", False):
            s11 = s11 * alpha + c11 * (1 - alpha)

        s12 = self.up_blocks[2](
            hidden_states=s11,
            temb=emb,
        ...
        if kwargs.get("decoder_hidden_states", False):
            s12 = s12 * alpha + c12 * (1 - alpha)

        # 6. post-process
        sample = self.conv_norm_out(s12)
        sample = self.conv_act(sample)
        sample = self.conv_out(sample)

        return sample, [s9, s10, s11, s12]
